    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    <h3 class="text-2xl font-bold text-gray-900">إدارة البراندات</h3>
                    <p class="text-gray-500 text-sm mt-1">إضافة وتعديل وحذف البراندات المتاحة.</p>
                </div>

                <div class="flex gap-3 items-center">
                    <!-- Search -->
                    <form action="{{ route('admin.brands.index') }}" method="GET" class="flex">
                    ...
                    <!-- Create Button -->
                    <a href="{{ route('admin.brands.create') }}" class="bg-[#E60914] hover:bg-red-700 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                    <p class="text-gray-500 text-xs">إجمالي البراندات</p>
                    <h4 class="text-xl text-gray-900 font-bold mt-1">{{ $totalBrands }}</h4>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
                    ...
                    <table class="w-full text-sm text-gray-700">
                        <thead class="bg-gray-50 text-gray-500 text-xs uppercase border-b border-gray-200">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-start">الرابط (URL)</th>
                                <th class="px-6 py-4 text-start">السنوات</th>
                                <th class="px-6 py-4 text-start">معلومات</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-gray-100 hover:bg-gray-50/50 transition duration-200">
                                ...
                                <td class="px-6 py-4">
                                    @if($brand->image)
                                    <img src="{{ Storage::url($brand->image) }}" alt="{{ $brand->name }}" class="w-12 h-12 rounded-lg object-cover shadow-sm">
                                    @else
                                ...
                                <td class="px-6 py-4 text-gray-900 font-medium">{{ $brand->name }}</td>

                                <td class="px-6 py-4 text-gray-600">
                                    @if($brand->url)
                                    <a href="{{ $brand->url }}" target="_blank" class="text-blue-500 hover:underline flex items-center gap-1">
                                        <i class="fas fa-external-link-alt text-xs"></i> زيارة
                                    </a>
                                    @else
                                ...
                                <td class="px-6 py-4 text-gray-600">{{ $brand->years ?? '-' }}</td>

                                <td class="px-6 py-4 text-gray-600 max-w-xs truncate">{{ $brand->info ?? '-' }}</td>

                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center gap-2 items-center flex-wrap">
                                        <!-- Edit -->
                                        <a href="{{ route('admin.brands.edit', $brand->id) }}" class="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 hover:bg-blue-600 hover:text-white transition" title="تعديل">
                                        ...
                                        <form action="{{ route('admin.brands.destroy', $brand->id) }}" method="POST" class="inline">
                                            ...
                                            <button type="submit" class="w-8 h-8 rounded-full bg-red-50 flex items-center justify-center text-red-600 hover:bg-red-600 hover:text-white transition"
                                                title="حذف"
                                                onclick="return confirm('هل أنت متأكد من حذف هذا البراند؟')">
                ...
                <div class="p-4 border-t border-gray-200 text-gray-600 text-sm flex justify-between items-center bg-gray-50/50">
                    <span>
                        عرض {{ $brands->firstItem() ?? 0 }} إلى {{ $brands->lastItem() ?? 0 }} من إجمالي {{ $brands->total() }}
                    </span>
                    {{ $brands->links() }}
                </div>
            </div>
